...
import engima.waratsea.model.squadron.deployment.DeploymentDAO;
import engima.waratsea.model.squadron.deployment.DeploymentMap;
...
public class SquadronAI {
    ...
    private DeploymentMap deploymentMap;                                             //Contains all the airfields for this nation.
     ...
     */
    @Inject
    public SquadronAI(final GameMap gameMap, final DeploymentDAO deploymentDAO, final DeploymentMap deploymentMap) {
    ...
    private void deployNation(final Nation nation) {
        ...
        try {
            ...
            List<Deployment> deployments = deploymentDAO.load(scenario, side, nation);            //The deployment for this nation.
    ...
    private void deploySquadron(final Squadron squadron, final List<Map.Entry<Integer, List<Airfield>>> sortedByRankings) {
        ...
        for (Map.Entry<Integer, List<Airfield>> entry : sortedByRankings) {
            ...
            for (Airfield airfield : entry.getValue()) {
                ...
                if (airfield.addSquadron(squadron)) {
                    log.info("Deploy squadron: '{}' of type '{}' to airfield: '{}'", new Object[]{squadron.getName(),  squadron.getBaseType(), airfield.getName()});
