...
public class VirtualPatrols {
     ...
     */
    @Inject
    public VirtualPatrols(final PatrolDAO patrolDAO) {
    ...
    public void build(final Airbase base, final PatrolsData data) {
        ...
        // Create virtual patrols for patrol types that can be virtual.
        PatrolType
                .virtualStream()
                .forEach(patrolType -> buildVirtualPatrol(patrolType, data));

        // Create empty patrols for patrol types that cannot be virtual.
        PatrolType
                .stream()
                .filter(type -> patrolMap.get(type) == null)
                .forEach(type -> patrolMap.put(type, new EmptyPatrol(type, airbase)));
    }
     ...
     */
    private void buildVirtualPatrol(final PatrolType patrolType, final PatrolsData patrolsData) {
        ...
        patrolMap.put(patrolType, patrolDAO.loadVirtual(patrolData));
    }
