...
public class AswPatrolSearchRules {
     ...
     */
    @Inject
    public AswPatrolSearchRules(final Weather weather,
                                final Dice dice) {
        ...
        searchFactor = SUB_SEARCH_FACTOR;
    }
    ...
    public Map<String, String> getBaseFactors(final int distance, final List<Squadron> squadrons) {
        ...
        factorMap.put("Step Factor (Seaplanes are doubled)", getStepFactor(steps) + "");
    ...
    private int getSteps(final List<Squadron> squadrons) {
        return squadrons
                .stream()
                .map(this::processStepFactor)
                .reduce(0, Integer::sum);
    }

    /**
     * Seaplane steps count as double for ASW searching. All other landing types are normal.
     *
